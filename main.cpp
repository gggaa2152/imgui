...
static void* SetupDiagnosticThread(void*) {
    ...
    // 绝不能仅依赖 dlopen 或 eglGetProcAddress，它们在中途注入时会返回当前线程的局部废弃指针
    void* egl_ptr = DobbySymbolResolver("libEGL.so", "eglSwapBuffers");
    void* egl_damage_ptr = DobbySymbolResolver("libEGL.so", "eglSwapBuffersWithDamageKHR");

    if (!egl_ptr) {
        LOGI("[-] DobbySymbolResolver failed, falling back to eglGetProcAddress...");
        egl_ptr = (void*)eglGetProcAddress("eglSwapBuffers");
    }
    if (!egl_damage_ptr) {
        egl_damage_ptr = (void*)eglGetProcAddress("eglSwapBuffersWithDamageKHR");
    }

    if (!egl_ptr) {
        LOGI("[-] eglGetProcAddress failed, falling back to dlopen...");
